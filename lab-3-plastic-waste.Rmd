...
```
answer:the africa is the most density continent and the south america is the most continent in plastic wate per cap
...
Answer here:`Because here we represent a set of data (continents),
not just color control, as the color here belong to the continents,and also the filling is related to the continents
But the Alpha it's talking about all Graphic(geme_density)
...
ggplot(data = plastic_waste, 
  ...
  geom_violin()
```
answer: box plots provide a coarser summary of the data, provide more of a summary of a distribution can also be seen sa an advantage in certain cases than violin plots.
...
geom_point()
```
Answre:Europe(positive) relationship,Africa is the most(mismanged_plastic waste per cap)
...
ggplot(data = plastic_waste,
    ...
    geom_point()
```
The answer: Asia has more population but less waste compared to continents with less population and more waste
...
plastic_waste %>% 
  ...
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) + 
    ...
    theme_minimal()
```
Answer: geme_smooth (Aids the eye in seeing patterns in the presence of overplotting also it’s help us to do see the curve of the data and analyis it via a different way)
